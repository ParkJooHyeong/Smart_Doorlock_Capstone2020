...
# Firebase DB, Storage data 수신, 송신
    ...
    if self.keypad is True:
        if len(self.password) == 6 and self.password == self.ans:
            ...
            if self.facerecog is True:
                ...
                if result_name == 'Unknown'  or float(result_accur) < 97:
...
def face(self):
    ...
    if names == 'Unknown' or float(accur) < 97:
        imagePath = "Faces/Pred/face.jpg"
        imageBlob = self.bucket.blob("User/007.jpg")
